""""
cont_gridworld_v6 as per updated api: https://www.gymlibrary.ml/content/environment_creation/
# Contnuous states. n Discrete Actions; multiple possible starting points; with velocity field
Similar to cont_gridworld_v6 but state_dim = 5: txyuv
"""
...
# Contnuous states. n Discrete Actions; multiple possible starting points; with velocity field
class ContGridWorld_v6(gym.Env):
    ...
    def __init__(self, state_dim=5, action_dim=1, action_range=[0.0,2*math.pi], grid_dim=[10.,10.],start_pos=[[5.0,5.0]], target_pos=[8.0,8.0], target_rad=1, F=1):
        ...
        print(f"**** initializing ContGridWorld_v6 environment *****")
        #default values
    ...
    def setup(self, cfg, params2, rzn=0):
        ...
        self.action_space = gym.spaces.Box(low=self.a_min, 
                                    high=self.a_max, 
                                    shape=(self.action_dim,))
        self.observation_space = gym.spaces.Box(low=np.array([0,0,0,-10,-10]), high=np.array([120, self.xlim, self.xlim, 10, 10]), shape=(self.state_dim,)) #square domain
    ...
    def has_reached_target(self):
        status = False
        if np.linalg.norm(self.state[1:3] - self.target_pos) <= self.target_rad:
            status = True
        return status
