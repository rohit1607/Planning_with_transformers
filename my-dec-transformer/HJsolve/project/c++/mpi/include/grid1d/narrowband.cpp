...
void Narrowband::split_communicator(){
    ...
    if(prev_active_proc!=active_proc_size or prev_row_wise!=row_wise){
...
void Narrowband::divide_contour(vector<vector<Point> > zero_contours){
    ...
    for (vector<vector<Point> >::iterator it=zero_contours.begin();it!=zero_contours.end();++it){
        for(vector<Point>::iterator sit=(*it).begin();sit!=(*it).end();++sit){
            int i=floor(sit->first);
            int j=floor(sit->second);
            row_counter[i]+=1;
            col_counter[j]+=1;
            if(i<iMin)
                iMin=i;
    ...
    int prev_rows_per_proc=no_rows;
    int prev_cols_per_proc=no_cols; 
    bool row_wise;
    //split row wise
    prev_row_wise=row_wise;
    ...
    prev_active_proc=active_proc_size;
    if(row_wise){
        int new_row_size=min(int(floor((no_rows-1e-5)/4))+1,proc_size);
        ...
        int new_col_size=min(int(floor((no_cols-1e-5)/4))+1,proc_size);
    ...
    if(row_wise){
        ...
        for(int r=0;r<active_proc_size;r++){
            col_split_vec.push_back(make_pair(max(0,jMin-nb_width),min(jdim-1,jMax+nb_width)));
        }
    }
    else{
        int cols_per_proc=round(csum/procshape[1]);
        int ends=cols_per_proc;
        int appended=0;
        int jmax=col_counter.rbegin()->first;
        int start=col_counter.begin()->first;
        for(map<int,int>::iterator it=col_counter.begin();it!=col_counter.end();++it){
            if((it->second>=ends)){
        ...
        for(int r=0;r<active_proc_size;r++){
            row_split_vec.push_back(make_pair(max(0,iMin-nb_width),min(idim-1,iMax+nb_width)));
        }
    }
}
