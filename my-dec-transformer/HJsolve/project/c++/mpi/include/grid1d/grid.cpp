...
void Grid::send_join_contours_full(vector<vector<vector<Point> > > zcs){
    ...
    if(row_wise){
        ...
        temp_zc=send_join_contours(zcs,procshape[0],llcomm);
        MPI_Comm_free(&llcomm);
    }
    else{
        color=cart_rank[0];
        MPI_Comm_split(lcomm,color,lrank,&llcomm);
        temp_zc=send_join_contours(zcs,procshape[1],llcomm);
        MPI_Comm_free(&llcomm);
    }
