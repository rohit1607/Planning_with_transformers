...
void Contouring::contouring_grid(){
    ...
    //cout<<"contouring contouring_grid"<<endl;
    for(auto it=v_narrowband.begin();it<v_narrowband.end();++it){
        int i=it->first;
        int j=it->second;
        if(i<idim && j<jdim){
            bool i_nxt=(s_narrowband.find(make_pair(i+1,j))!=s_narrowband.end());
            bool j_nxt=(s_narrowband.find(make_pair(i,j+1))!=s_narrowband.end());
            bool ij_nxt=(s_narrowband.find(make_pair(i+1,j+1))!=s_narrowband.end());
...
bool Equal(Point p1,Point p2){
    ...
    if(pow((pow((p1.first-p2.first),2))+(pow((p1.second-p2.second),2)),.5)<1e-4){
        return true;
    }
        return false;
}
...
list<Edge> eliminate_duplicates(list<Edge> Edges_list){
    list<Edge> unique_edge;
   ...
   // cout<<"checking duplicates"<<endl;
    for(auto it=Edges_list.begin();it!=Edges_list.end();it++){
        ...
        bool found=false;
        if(!Equal(it->one,it->two)){
             for(auto sit=unique_edge.begin();sit!=unique_edge.end();sit++){
        ...
        if(not found){
            Edge e=*it;
            unique_edge.push_back(e);
        }
    }
    Edges_list.clear();
    Edges_list.assign(unique_edge.begin(),unique_edge.end());
    return Edges_list;
}
...
vector<vector< Point > > Contouring::get_contours(){
    ...
    Edges_list=eliminate_duplicates(Edges_list);
